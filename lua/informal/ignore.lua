    ...
    if formatter_pragma_comments then
      if start_line == end_line then -- one line selected
        if formatter_pragma_comments.before then
          ...
          local comment_string = indent .. utils.resolve_pragma_comment(formatter_pragma_comments.before)
          vim.api.nvim_buf_set_lines(0, start_line - 1, start_line - 1, true, { comment_string })
        elseif formatter_pragma_comments.inline then
          local comment_string = " " .. utils.resolve_pragma_comment(formatter_pragma_comments.inline)
        ...
        vim.api.nvim_buf_set_lines(0, start_line - 1, start_line - 1, true, {
          indent_start .. utils.resolve_pragma_comment(comment_string[1]),
        })
        vim.api.nvim_buf_set_lines(0, end_line + 1, end_line + 1, true, {
          indent_end .. utils.resolve_pragma_comment(comment_string[2]),
        })
      end
    end
  end
